...
test('Tool Groups: starts in core mode and enables groups on demand', () => {
  ...
  assert.equal(manager.isToolEnabled('playlite_run'), true);
  ...
  assert.equal(manager.isToolEnabled('playlite_codemod'), false);
  ...
  // Enable all
  manager.enable(['all']);
  assert.equal(manager.isToolEnabled('playlite_codemod'), true);
});
